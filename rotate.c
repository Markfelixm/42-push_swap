...
/*   Updated: 2023/03/14 14:44:40 by marmulle         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_op	ra(t_stacks *stacks)
{
	if (stacks->a_len < 2)
		return (RA);
	stacks->a[get_index(stacks, stacks->a_start + stacks->a_len)] = stacks->a[stacks->a_start];
...
t_op	rb(t_stacks *stacks)
{
	if (stacks->b_len < 2)
		return (RB);
	stacks->b[get_index(stacks, stacks->b_start + stacks->b_len)] = stacks->b[stacks->b_start];
